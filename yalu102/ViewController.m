...
NSInteger nosubstrateisactivenowwhat;
NSInteger nosubstrateplease;

- (IBAction)substratetoggle:(id)sender {
    if ([_substrateswitch isOn]) {
        nosubstrateplease = 0;
        [[NSUserDefaults standardUserDefaults] setBool:self.substrateswitch.on forKey:@"substrater"];
    ...
    NSString *status = @"ON";
    NSUserDefaults *substrater = [NSUserDefaults standardUserDefaults];
    if(!_substrateswitch.on){
        status = @"OFF";
        [substrater setObject:status forKey:@"substrater"];
    }
    [substrater setObject:status forKey:@"substrater"];
    //Thanks @1GamerDev for the idea!
   }

- (void)viewDidLoad {
    
    [super viewDidLoad];
    NSString *labels = [[NSUserDefaults standardUserDefaults] stringForKey:@"label"];
    if([labels compare:@"YES"] == NSOrderedSame){
        _nosubstrate.enabled = YES;
    }
    NSString *substrater = [[NSUserDefaults standardUserDefaults] stringForKey:@"substrater"];
        if([substrater compare:@"ON"] == NSOrderedSame){
...
gotclock:;
    ...
    *(uint64_t*)(((uint64_t)faketask) + 0x2b8) = itk_kern_sself;
    ...
    [dope setEnabled:NO];
    [dope setTitle:@"Jailbroken" forState:UIControlStateDisabled];
        if(nosubstrateplease == 1){
            UIAlertController * alert = [UIAlertController
                                         alertControllerWithTitle:@"Jailbroken!"
                                         message:@"The Jailbreak has been activated but without Mobile Substrate, so tweaks won't work! No respring will be made."
                                         preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction* yup = [UIAlertAction
